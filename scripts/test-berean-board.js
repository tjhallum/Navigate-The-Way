...
test('renders group setup wizard controls before lesson setup in the browser form', () => {
  ...
  assert.match(html, /<link rel="stylesheet" href="styles\.css\?v=20260620-override-label-space" \/>/);
  ...
  assert.match(html, /<script src="berean-board\.js\?v=20260620-override-label-space"><\/script>/);
...
test('hides host override controls until a contestant has an answer verdict', () => {
  ...
  assert.match(css, /\.contestant-choice--has-host-overrides\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+auto/i);
  ...
  assert.match(cssRule(css, '.contestant-choice__host-overrides'), /justify-self:\s*end/i);
  ...
  assert.match(cssRule(css, '.contestant-choice__host-override-button'), /width:\s*1\.55rem/i);
  assert.match(cssRule(css, '.contestant-choice__host-override-button'), /height:\s*1\.55rem/i);
