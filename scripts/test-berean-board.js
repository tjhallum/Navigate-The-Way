...
test('renders group setup wizard controls before lesson setup in the browser form', () => {
  ...
  assert.match(html, /<link rel="stylesheet" href="styles\.css\?v=20260620-bottom-override-icons" \/>/);
...
test('hides host override controls until a contestant has an answer verdict', () => {
  ...
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides'), /padding-block-end:\s*1\.95rem/i);
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides .contestant-choice__label'), /min-width:\s*0/i);
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides .contestant-choice__label'), /gap:\s*0\.34rem/i);
  assert.match(cssRule(css, '.contestant-choice__host-overrides'), /position:\s*absolute/i);
  assert.match(cssRule(css, '.contestant-choice__host-overrides'), /inset-block-end:\s*0\.35rem/i);
  assert.match(cssRule(css, '.contestant-choice__host-overrides'), /inset-inline-start:\s*50%/i);
  assert.match(cssRule(css, '.contestant-choice__host-overrides'), /transform:\s*translateX\(-50%\)/i);
  ...
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides .contestant-choice__body'), /gap:\s*0\.03rem/i);
  ...
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides .contestant-choice__name'), /font-size:\s*clamp\([^)]*cqi/i);
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides .contestant-choice__name'), /line-height:\s*1\.05/i);
  ...
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides small'), /font-size:\s*clamp\([^)]*cqi/i);
  assert.match(cssRule(css, '.contestant-choice--has-host-overrides small'), /line-height:\s*1\.05/i);
